...
class Users extends Controller
{
    ...
    public function createEvent()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (
                empty($data['name_error']) && empty($data['organizer_error']) && empty($data['location_error'])
    ...
    public function pwrecovery()
    {
        ...
        $valid = false;//Variable needed for user authentication
        ...
        if (isset($_POST['username'])) {
            ...
            $_SESSION["a"] = $_POST['username'];//using this allows me to access the username in the second if statement for the second form
        ...
        if ($valid) { //if user has authenticated themselves, the second form is displayed on the page.
        ...
        if (isset($_POST['passwordconf'])) {
            ...
            $CheckDaTing = $this->userModel->changepw($data2['password'], $data2['passwordconf'], $username);//Passes the password to be checked and if both are the same will update
